  ...
  return (
    <div className="fixed inset-0 z-[100] bg-white flex flex-col animate-in slide-in-from-right duration-300">
      ...
      <header className="px-6 pt-12 pb-6 flex items-center justify-between border-b border-slate-50">
        ...
        <h1 className="text-xl font-black text-slate-800 tracking-tight">完善个人信息</h1>
        <div className="w-10">
          {!isEditMode && (
      ...
      <main className="flex-1 overflow-y-auto px-6 py-4 space-y-8">
        ...
        <div className="space-y-4">
          {/* Nickname Input */}
          <div className="bg-white border border-slate-100 rounded-[32px] p-6 shadow-sm focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all">
            <input
              type="text"
              value={formData.name}
              onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              placeholder="请输入昵称"
              className="w-full bg-transparent font-bold text-slate-800 outline-none placeholder:text-slate-300 text-lg"
            />
          </div>

          {/* Birth Date Picker */}
          <button className="w-full bg-white border border-slate-100 rounded-[32px] p-6 flex items-center justify-between shadow-sm active:scale-[0.98] transition-all group">
            <span className="font-bold text-slate-800 text-lg">出生日期</span>
            <div className="flex items-center space-x-2 text-slate-400 group-hover:text-emerald-500">
          ...
          {/* Gender Picker */}
          <button className="w-full bg-white border border-slate-100 rounded-[32px] p-6 flex items-center justify-between shadow-sm active:scale-[0.98] transition-all group">
            <span className="font-bold text-slate-800 text-lg">性别</span>
            <div className="flex items-center space-x-2 text-slate-400 group-hover:text-emerald-500">
          ...
          {/* Height Picker */}
          <button className="w-full bg-white border border-slate-100 rounded-[32px] p-6 flex items-center justify-between shadow-sm active:scale-[0.98] transition-all group">
            <span className="font-bold text-slate-800 text-lg">身高</span>
            <div className="flex items-center space-x-2 text-slate-400 group-hover:text-emerald-500">
          ...
          {/* Weight Picker */}
          <button className="w-full bg-white border border-slate-100 rounded-[32px] p-6 flex items-center justify-between shadow-sm active:scale-[0.98] transition-all group">
            <span className="font-bold text-slate-800 text-lg">体重</span>
            <div className="flex items-center space-x-2 text-slate-400 group-hover:text-emerald-500">
