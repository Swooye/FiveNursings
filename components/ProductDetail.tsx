...
const ProductDetail: React.FC<ProductDetailProps> = ({ sku, onBack, onPurchase, onAddToCart }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-white dark:bg-slate-950 flex flex-col overflow-y-auto animate-in slide-in-from-right duration-300 no-scrollbar">
      {/* Toast Notification */}
      {showToast && (
      ...
      {/* Dual Action Bar */}
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-white/90 dark:bg-slate-950/90 backdrop-blur-2xl border-t border-slate-100 dark:border-slate-800 z-50 flex items-center space-x-4">
        <button 
          onClick={handleAdd}
        ...
        {/* Reverted to Emerald Green for the CTA button */}
        <button 
        ...
        >
          <span>立即结算 ¥{sku.price * quantity}</span>
